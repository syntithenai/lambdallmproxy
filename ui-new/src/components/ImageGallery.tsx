    ...
    <div className="my-4 flex gap-2 flex-wrap">
      {displayedImages.map((imageUrl, idx) => {
        ...
        return (
          <div 
            key={`${imageUrl}-${idx}`}
            className="relative group rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700 hover:border-blue-500 dark:hover:border-blue-400 transition-colors"
          >
            <img 
              src={imageUrl} 
            ...
            {onGrabImage && (
              <button
                ...
                className="absolute bottom-1 right-1 opacity-0 group-hover:opacity-100 transition-opacity bg-blue-600 hover:bg-blue-700 text-white text-xs px-2 py-1 rounded shadow-lg"
                title="Add to SWAG"
              >
                📎 Grab
              </button>
