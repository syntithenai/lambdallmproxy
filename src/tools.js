...
const toolFunctions = [
  ...
  // Browser feature tools (client-side execution)
  {
    type: 'function',
    function: {
      name: 'browser_storage_read',
      description: '📦 **READ BROWSER STORAGE** (Client-side): Read data from localStorage or sessionStorage in the user\'s browser. Use when user asks to "read", "get", "check", or "show" localStorage/sessionStorage values.',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description: '💾 **WRITE BROWSER STORAGE** (Client-side): Save data to localStorage or sessionStorage in the user\'s browser. Use when user asks to "save", "store", "write", or "set" a value in storage.',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description: '📋 **COPY TO CLIPBOARD** (Client-side): Copy text to the user\'s clipboard. Use when user asks to "copy" text or save something to clipboard.',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description: '📋 **READ CLIPBOARD** (Client-side): Read text from the user\'s clipboard. Use when user asks to "paste", "read clipboard", or "what\'s in my clipboard".',
      parameters: {
        type: 'object',
        properties: {},
        additionalProperties: false
  ...
  },
  {
    ...
    function: {
      ...
      description: '🔔 **SHOW BROWSER NOTIFICATION** (Client-side): Display a desktop notification to the user. Use when user asks to "notify", "alert", "remind", or "show notification".',
      parameters: {
        type: 'object',
        properties: {
  ...
  },
  {
    ...
    function: {
      ...
      description: '📍 **GET USER LOCATION** (Client-side): Get the user\'s current geographic coordinates (latitude/longitude). Use when user asks for "my location", "coordinates", "where am I", or geolocation data.',
      parameters: {
        type: 'object',
        properties: {},
        additionalProperties: false
  ...
  },
  {
    ...
    function: {
      ...
      description: '🔍 **QUERY PAGE ELEMENTS** (Client-side): Find and extract information from HTML elements on the current page. Use when user asks to "find links", "get page elements", "query DOM", or extract data from the page.',
      parameters: {
        type: 'object',
        properties: {
...
After generating the chart, continue with any explanation or additional information.`
    ...
    case 'browser_query_dom': {
      // These tools execute in the browser, not on the server
      // Return a special marker so the frontend knows to execute them client-side
      console.log(`🌐 BROWSER TOOL: ${name} - marking for client-side execution`);
      return JSON.stringify({
        client_side: true,
        tool_name: name,
        args: args,
        message: 'This tool will execute in the browser'
      });
    }
