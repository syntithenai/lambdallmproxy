 ...
 * Supports Unsplash, Pexels, and AI-generated images (Together AI FLUX.1-schnell-Free)
 */
 ...
 * @param {number} imageRequest.index - Index in batch (0-based)
 * @returns {Promise<Object>} Image data with base64 encoding
 */
async function fetchSingleImage(imageRequest) {
    const { searchTerms, itemId, source = 'auto', index = 0 } = imageRequest;
    
    try {
        console.log(`🖼️ Fetching image for "${searchTerms}" (source: ${source}, index: ${index})`);
        
        // Generate AI image for the FIRST item only (index 0)
        if (index === 0 && (source === 'ai' || source === 'auto')) {
            try {
                console.log(`🎨 Generating AI image for: "${searchTerms}"`);
                const aiImageResult = await generateTogetherImage({
                    prompt: searchTerms,
                    model: 'black-forest-labs/FLUX.1-schnell-Free', // FREE model
                    width: 1024,
                    height: 768,
                    steps: 4, // Fast generation (4 steps for schnell)
                    apiKey: process.env.LP_KEY_3 // Together AI key from .env
                });
                
                if (aiImageResult && aiImageResult.success && aiImageResult.base64) {
                    console.log(`✅ AI image generated successfully`);
                    const dataUri = `data:image/png;base64,${aiImageResult.base64}`;
                    return {
                        itemId,
                        success: true,
                        image: dataUri,
                        ...
                        photographerUrl: 'https://www.together.ai',
                        attribution: 'AI Generated by Together AI',
                        attributionHtml: 'AI Generated by <a href="https://www.together.ai" target="_blank">Together AI</a>'
                    };
                }
            } catch (aiError) {
                console.warn(`⚠️ AI generation failed, falling back to photo search:`, aiError.message);
                // Fall through to photo search
            }
        }
        
        // Search for image using existing image search tools (Unsplash/Pexels)
    ...
    try {
        ...
        for (let i = 0; i < images.length; i += BATCH_SIZE) {
            ...
            const batchResults = await Promise.all(
                batch.map((imageRequest, batchIndex) => 
                    fetchSingleImage({ ...imageRequest, index: i + batchIndex })
                )
            );
