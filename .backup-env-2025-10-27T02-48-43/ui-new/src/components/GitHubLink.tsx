...
export const GitHubLink: React.FC<GitHubLinkProps> = ({ onOpenSettings, hideGitHub = false }) => {
  const navigate = useNavigate();
  const location = useLocation();
  const isOnHelpPage = location.pathname === '/help' || location.pathname === '/privacy';
  ...
  return (
    <div className="hidden md:flex fixed bottom-4 right-4 flex-col gap-2 z-50" style={{ zIndex: 9999 }}>
      ...
      {/* Help Button - Only show when not on help/privacy pages and when onOpenSettings provided */}
      {onOpenSettings && !isOnHelpPage && (
        <button
